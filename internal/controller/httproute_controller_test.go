...
func TestHTTPRouteReconciler_AppliesDefaultTagsEvenWithoutOwnTags(t *testing.T) {
	ctx := context.Background()
	r, fake := newHTTPRouteReconciler(false)
	...
	route := &gatewayv1.HTTPRoute{
		...
		Spec: gatewayv1.HTTPRouteSpec{Hostnames: []gatewayv1.Hostname{"default-tags.example.com"}},
	}
	if err := k8sClient.Create(ctx, route); err != nil {
		t.Fatalf("create HTTPRoute: %v", err)
	}
	...
	if len(fake.MonitorTags[id]) != 1 {
		t.Errorf("MonitorTags[id] = %v, want the 1 default tag applied even though the HTTPRoute itself declares none", fake.MonitorTags[id])
	}
}

func TestHTTPRouteReconciler_PersistsMonitorIDsDespiteTagSyncFailure(t *testing.T) {
	ctx := context.Background()
	r, fake := newHTTPRouteReconciler(true)
	...
	route := &gatewayv1.HTTPRoute{
		...
		Spec:       gatewayv1.HTTPRouteSpec{Hostnames: []gatewayv1.Hostname{"tagfail.example.com"}},
	}
	if err := k8sClient.Create(ctx, route); err != nil {
		t.Fatalf("create HTTPRoute: %v", err)
	}
